...
def main(argv=None):
    ...
    if not args.say: parser.error("--say is required")
    client = ChatboxClient(host=SETTINGS.vrchat_ip,port=SETTINGS.osc_in_port,max_len=SETTINGS.chatbox_max_len,debug=SETTINGS.debug)
    try:
        client.typing(True); client.say(args.say, press_enter=True)
    finally:
        client.typing(False)
